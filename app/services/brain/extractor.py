...
- slot_deltas: facts from the lead's MOST RECENT message(s), extracted the same way whatever language she writes in. Fields already listed under KNOWN SO FAR are context only; do not re-populate them unless the lead restated or changed them this turn.
  ...
  - partner_is_decision_maker: whether her PARTNER makes or shares the decision - the field is about the partner, NOT about her. true when the partner decides or must agree ("I'll have to ask my partner", "it's my partner's decision"). false when she explicitly states SHE alone decides ("I'm the only decision maker" -> false, "I decide about this myself" -> false, the partner is NOT a decision maker in those cases). NEVER infer it from the partner being unwilling or unable to join the call ("he doesn't want to join" says nothing about who decides -> null). partner_can_join: only if stated.
...
- intent: classify the LEAD's most recent message (one label). Use booked when she says she HAS scheduled/booked/reserved the call, in any phrasing and whatever tense ("booked!", "just booked it for Tuesday", "all set for Thursday", "I picked a time", "done, see you then", "ya agendé"). Use ready_to_book when she only says she WANTS to book but has not yet ("send me the link", "how do I book?") - the difference is whether the booking already happened. Use gives_email when the message is (or contains) her email address. Use ivf_only when she says a doctor told her IVF is her only/best/last option or that she can only conceive through IVF (also set treatment_path="ivf"; do NOT set ivf_interest from this). Use not_ready_no_money when the lead clearly cannot afford it, has no money, has a very tight budget, wants only free help, or is not ready to commit at all. If she says her budget is tight / she can't afford it AND also asks the price in the same message, still classify it as not_ready_no_money (the affordability concern dominates). Do NOT use not_ready_no_money for "I need to ask/consult my partner" or "it's my partner's decision" - classify those as answers_question and set partner_status=couple and partner_is_decision_maker=true.
