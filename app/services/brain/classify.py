...
- answers_question: she is responding to something Sonia asked. This is the DEFAULT whenever her message follows a question from Sonia and no more specific label above clearly applies. It covers bare answers - a number, an age, a date, "yes", "no", "not yet", "still deciding". Prefer this over "other" every time there is a question in front of it. Never use it for a message that states an objection, a fear, a question or her own history: every label above is checked first. answers_question requires something to be answering - a question from Sonia in the transcript, or a message that plainly reads as a reply. It is NOT the residual label: a message with no content of its own and no question in front of it is "other" with certainty unsure, never an answer.
