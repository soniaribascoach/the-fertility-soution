...
- answers_question: she is responding to something Sonia asked. This is the DEFAULT whenever her message follows a question from Sonia and no more specific label above clearly applies. It covers bare answers - a number, an age, a date, "yes", "no", "not yet", "still deciding". Prefer this over "other" every time there is a question in front of it. BUT: when the transcript shows no previous Sonia turn, only a bare answer of that kind may be answers_question. A message that carries its own substance - a fear, an objection, a question, her history - is never answers_question just because it could be read as replying to something. Label what she is actually doing.
